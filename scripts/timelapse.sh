...
mkdir ~/allsky/images/$1/sequence/
# find images, make symlinks sequentially and start avconv to build mp4; upload mp4 and move directory
find "~/allsky/images/$1" -name "*.$EXTENSION" -size 0 -delete
ls -rt ~/allsky/images/$1/*.$EXTENSION |
gawk 'BEGIN{ a=1 }{ printf "ln -sv %s ~/allsky/images/'$1'/sequence/%04d.'$EXTENSION'\n", $0, a++ }' |
bash
...
rm -rf ~/allsky/images/$1/sequence
